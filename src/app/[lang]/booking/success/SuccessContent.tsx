...
export default function SuccessContent({ dict, lang }: { dict: any, lang: string }) {
  ...
  return (
    <div className="max-w-xl w-full bg-white p-10 rounded-2xl border border-sand shadow-sm text-center space-y-6 relative overflow-hidden">
      ...
      <h1 className="text-3xl font-serif text-ocean relative z-10">{dict.success?.title}</h1>
      ...
      {sessionId && (
        <p className="text-xs text-gray-400 mt-4 relative z-10">{dict.success?.ref} {sessionId.substring(0, 15)}...</p>
      )}

      <div className="pt-6 border-t border-gray-100 flex flex-col space-y-3 relative z-10 mt-8">
        <Link href={`/${lang}`} className="w-full bg-ocean text-white py-4 rounded font-medium hover:bg-ocean-light transition block">
          {dict.success?.return}
        </Link>
      </div>
    </div>
